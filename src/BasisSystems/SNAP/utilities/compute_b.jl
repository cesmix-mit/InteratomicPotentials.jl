    ...
    for ielem = 1:num_elements
        ...
        if (snap.bzero_flag) 
            if (~snap.wselfall_flag) 
                itriple = (ielem-1)*( num_elements+1)*num_elements+(ielem-1);
                for jjb = 0:(snap.prebuilt_arrays.idxb_max-1)
                    j = snap.prebuilt_arrays.idxb[jjb+1].j;
...
function compute_dbidrj(jelem::Int, snap::SNAP, runtime_arrays::RuntimeArrays)
    ...
    n_rows, n_cols = size(runtime_arrays.dblist)
    for row = 1:n_rows
        for col = 1:n_cols
            runtime_arrays.dblist[row, col] = 0.0
        end
    end
    ...
    for jjb = 0:(snap.prebuilt_arrays.idxb_max-1) 
        ...
        for elem1 = 0:(num_elements-1) 
            for elem2 = 0:(num_elements-1)
                ...
                jjz = snap.prebuilt_arrays.idxz_block[j1+1,j2+1,j+1];
                jju = snap.prebuilt_arrays.idxu_block[j+1];
                idouble = elem1*num_elements+elem2;
                itriple = (elem1*num_elements+elem2)*num_elements+elem3;
                zptr_r = runtime_arrays.zlist_r[idouble*snap.prebuilt_arrays.idxz_max+1:end];
                zptr_i = runtime_arrays.zlist_i[idouble*snap.prebuilt_arrays.idxz_max+1:end];
                sumzdu_r = zeros(3)
                ...
                for k = 1:3
                    runtime_arrays.dblist[itriple*snap.prebuilt_arrays.idxb_max+jjb+1, k] = 2.0 * sumzdu_r[k] 
                end

                # // Sum over Conj(dudr(j1,ma1,mb1))*z(j,j2,j1,ma1,mb1)
                j1fac   = (j + 1) / (j1 + 1.0);
                idouble = elem1*num_elements+elem2;
                itriple = (elem3*num_elements+elem2)*num_elements+elem1;
                ...
                for k = 1:3
                    if (snap.bnorm_flag)
                        ...
                        runtime_arrays.dblist[itriple*snap.prebuilt_arrays.idxb_max+jjb+1, k] += 2.0 * sumzdu_r[k] * j1fac;
                    end
                end
                ...
                idouble = elem2*num_elements+elem1;
                itriple = (elem1*num_elements+elem3)*num_elements+elem2;
                ...
                for k = 1:3 
                    if (snap.bnorm_flag)
                        ...
                        runtime_arrays.dblist[itriple*snap.prebuilt_arrays.idxb_max+jjb+1, k] += 2.0 * sumzdu_r[k] * j2fac;
                    end
                end
            end
        end
